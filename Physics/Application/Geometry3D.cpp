...
Point ClosestPoint(const AABB& aabb, const Point& point) {
	...
	result.x = (result.x < min.x) ? min.y : result.x;
...
bool PointOnRay(const Point& point, const Ray& ray) {
	...
	//If BOTH vectors point in the same direction,dff should be 1
...
bool Raycast(const OBB& obb, const Ray& ray, RaycastResult* outResult) {
	...
	for (int i = 0; i < 3; ++i) {
		...
		t[i * 2 + 1] = (e[i] - size[i]) / f[i]; //tmin[x，y，z]
	}
...
//没有accelerator，则用每个三角形的AABB检测
//有accelerator，用accelerator检测
