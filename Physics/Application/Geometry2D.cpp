...
bool LineCircle(const Line2D& line, const Circle& circle)
{
	...
	//then the closest point was outide the segmanet
	if (t < 0.0f || t > 1.0f) {
		return false;
	}
...
bool LineRectangle(const Line2D& line, const Rectangle2D& rect) {
	...
	float tmin = fmaxf(fminf(min.x, max.y), fminf(min.y, max.y));
...
bool CircleRectangle(const Circle& circle, const Rectangle2D& rect) {
	Point2D closetPoint = circle.position;
	vec2 min = GetMin(rect);
	vec2 max = GetMax(rect);
	closetPoint.x = (closetPoint.x < min.x) ? min.x :
		(closetPoint.x > max.x) ? max.x : closetPoint.x;
	closetPoint.y = (closetPoint.y < min.y) ? min.y :
		(closetPoint.y > max.y) ? max.y : closetPoint.y;

	Line2D line(circle.position, closetPoint);
